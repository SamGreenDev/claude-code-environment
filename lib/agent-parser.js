...
/**
 * Walk plugin cache directories (local + cache), calling `loader(pluginPath, pluginName, results)`
 * for each discovered plugin. Handles the vendor/plugin/version nesting in cache dirs.
 * Extracted to eliminate ~120 lines of copy-pasted traversal code.
 */
async function traversePluginCache(loader) {
  const results = [];
  const pluginDirs = [
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (baseDir.includes('cache')) {
          try {
            const vendorEntries = await readdir(pluginPath, { withFileTypes: true });
            for (const pluginEntry of vendorEntries) {
              if (!pluginEntry.isDirectory() || pluginEntry.name.startsWith('.')) continue;
              const pluginSubDir = join(pluginPath, pluginEntry.name);
              const versionEntries = await readdir(pluginSubDir, { withFileTypes: true });
              for (const versionEntry of versionEntries) {
                if (!versionEntry.isDirectory()) continue;
                await loader(join(pluginSubDir, versionEntry.name), pluginEntry.name, results);
              }
            }
          } catch {
        ...
        await loader(pluginPath, entry.name, results);
      }
    } catch {
...
async function getPluginAgents() {
  return traversePluginCache(loadPluginAgentsFromPath);
}
...
function parseAgentFile(filename, content, pluginName = null) {
  ...
  if (fmMatch) {
    ...
    for (const line of fmLines) {
      ...
      if (kvMatch) {
        ...
        // If val is empty, next lines may be a list
      }
    }
    // Flush trailing list
    if (currentKey && currentList) {
  ...
  return {
    ...
    whenToUse,
    capabilities,
    allowedTools: Array.isArray(frontmatter.allowed_tools) ? frontmatter.allowed_tools : null,
...
async function getPluginSkills() {
  return traversePluginCache(loadPluginSkillsFromPath);
}
...
function parseSkillFile(name, content, pluginName = null) {
  ...
  const allowedToolsRaw = frontmatter['allowed-tools'] || frontmatter['allowed_tools'] || '';
...
async function getPluginCommands() {
  return traversePluginCache(loadPluginCommandsFromPath);
}
