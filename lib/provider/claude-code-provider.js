...
export class ClaudeCodeProvider extends BaseProvider {
  ...
  async executeNode(node, context, runId) {
    ...
    const args = [
      ...
      '--allowedTools', 'Edit', 'Write', 'Read', 'Glob', 'Grep', 'Bash', 'NotebookEdit',
    ];
      ...
      try {
        ...
        try {
          const fallback = {
            ...
            output: code === 0 ? (fullOutput || lastText || 'Completed') : null,
            error: code !== 0 ? `Process exited with code ${code}` : null,
          };
          writeFileSync(taskPath, JSON.stringify(fallback, null, 2), 'utf8');
        } catch (err2) {
