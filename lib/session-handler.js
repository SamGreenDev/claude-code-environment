...
// Async exec to avoid blocking the event loop during process inspection
 ...
 * Now async since isProcessAlive uses async exec.
 */
...
async function isProcessAlive(pid) {
  if (!pid) return false;
  try {
    process.kill(pid, 0); // Sync signal check — fast, no spawn
 ...
 * Fully async to avoid blocking the event loop (pgrep/ps/lsof can take seconds).
 * @returns {Promise<Array>} Array of discovered sessions
 */
export async function scanForSessions() {
  ...
  try {
    ...
    const { stdout: psOutput } = await execFile('pgrep', ['-x', 'claude'], {
      timeout: 5000
    });
    ...
    for (const pidLine of pids) {
      ...
      if (!existingSession) {
        ...
        try {
          const { stdout: ttyOutput } = await execFile('ps', ['-p', String(pid), '-o', 'tty='], {
            timeout: 1000
          });
          ...
          if (tty && tty !== '??' && tty !== '-') {
            ...
            // Get working directory via lsof — uses shell for pipe/grep/awk
            let cwd = '';
            try {
              const { stdout: lsofOut } = await execFile('sh', ['-c', `lsof -p ${pid} 2>/dev/null | grep cwd | awk '{print $9}'`], {
                timeout: 1000
              });
              cwd = lsofOut.trim();
            } catch {
 ...
 * Async to avoid blocking during process checks.
 */
...
function startPeriodicScan() {
  ...
  periodicScanTimer = setInterval(async () => {
    try {
      const discovered = await scanForSessions();
...
// Load persisted state on module initialization (fire-and-forget; async since isProcessAlive is async)
loadState().catch(err => console.error('[session] State load failed:', err.message));
