...
function sanitizeDescription(text) {
  ...
  // Strip "You are teammate 'X' on team 'Y'." preamble
  clean = clean.replace(/^You are teammate ["'][^"']+["'] on team ["'][^"']+["']\.\s*/i, '');

  // Strip "Check the task list..." and similar boilerplate
  clean = clean.replace(/^Check the task list[^.]*\.\s*/i, '');

  // If after stripping we have nothing meaningful, use a generic message
  if (!clean || clean.length < 3) return 'Working on task...';

  // Truncate to 60 chars for clean bubble display
  if (clean.length > 60) {
