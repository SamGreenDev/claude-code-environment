...
import { getAllLiveTeammates } from './team-scanner.js';
...
// Correlation state for PreToolUse[Task] → SubagentStart
const taskMetadata = new Map();
const METADATA_TTL = 30000; // 30 seconds
...
function loadState() {
  try {
    if (existsSync(STATE_FILE)) {
      ...
      if (data.agents && Array.isArray(data.agents)) {
        for (const agent of data.agents) {
          ...
          if (['working', 'walking', 'spawning'].includes(agent.status)) {
            // Promote to 'working' — no point replaying walk animation after restart
            agent.status = 'working';
            activeAgents.set(agent.id, agent);
            // Track restored teammates so syncTeammates() can clean up stale ones
            if (agent._isTeammate) {
              trackedTeammates.add(agent.id);
            }
      ...
      // Recreate auto-expiration timers for restored agents
      for (const [id, agent] of activeAgents) {
        const elapsed = Date.now() - new Date(agent.spawnedAt).getTime();
        const maxLifetime = (agent.source === 'team' || agent._isTeammate) ? TEAMMATE_MAX_LIFETIME : AGENT_MAX_LIFETIME;
        const remaining = maxLifetime - elapsed;
        if (remaining <= 0) {
          // Already expired — remove immediately
          console.log(`[activity] Auto-expiring stale restored agent: ${id}`);
...
// Longer lifetime for tmux teammates (4 hours)
const TEAMMATE_MAX_LIFETIME = 4 * 60 * 60 * 1000;

// How often to scan for tmux teammates (10 seconds)
const TEAM_SCAN_INTERVAL = 10000;

// Track agent expiration timers
const expirationTimers = new Map();

// Track which tmux teammates we've already spawned Jedi for
const trackedTeammates = new Set();

// Team scanner interval reference
let teamScanTimer = null;
...
/**
 * Sanitize task description for display in speech bubbles.
 * Strips verbose prompt preambles and truncates to a readable length.
 * @param {string} text - Raw description text
 * @returns {string} Cleaned description
 */
function sanitizeDescription(text) {
  if (!text) return 'Working on task...';
  let clean = text;

  // Phase 1 — Strip boilerplate preambles (applied sequentially)
  clean = clean.replace(/^You are teammate ["'][^"']+["'] on team ["'][^"']+["']\.\s*/i, '');
  clean = clean.replace(/^Check the task list[^.]*\.\s*/i, '');
  clean = clean.replace(/^You are (?:a|an) [^.]*\.\s*/i, '');
  clean = clean.replace(/^Your (?:job|role|task|goal|purpose) is to [^.]*\.\s*/i, '');
  clean = clean.replace(/^IMPORTANT:\s*[^.]*\.\s*/g, '');
  clean = clean.replace(/^(?:NOTE|CRITICAL|WARNING):\s*[^.]*\.\s*/g, '');
  clean = clean.replace(/^As (?:a|an|the) [^,]+,\s*(?:you|your)[^.]*\.\s*/i, '');
  clean = clean.replace(/^This (?:agent|task|tool)[^.]*\.\s*/i, '');
  // Second pass for chained preambles
  clean = clean.replace(/^You are (?:a|an) [^.]*\.\s*/i, '');
  clean = clean.replace(/^Your (?:job|role|task|goal|purpose) is to [^.]*\.\s*/i, '');

  // Phase 2 — Extract first sentence
  clean = clean.trim();
  ...
  // Phase 3 — Validate + truncate
  if (!clean || clean.length < 3) return 'Working on task...';
  if (clean.length > 60) {
...
export function spawnAgent(data) {
  const { agentId, agentType, taskDescription, prompt, subagentType, sessionId, source, teamName, memberName } = data;

  // Look up correlated metadata from PreToolUse[Task]
  // Don't delete — multiple agents may share a sessionId. TTL cleanup handles expiry.
  let metadata = null;
  if (sessionId && taskMetadata.has(sessionId)) {
    metadata = taskMetadata.get(sessionId);
    console.log(`[activity] Found correlated metadata for session ${sessionId}`);
  }

  // Use provided agentId or generate one
  const id = agentId || `agent-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

  // Priority: metadata.subagentType > subagentType > agentType > default
  const type = (metadata?.subagentType) || subagentType || agentType || 'general-purpose';
  ...
  const agent = {
    ...
    taskDescription: sanitizeDescription((metadata?.description) || taskDescription || prompt),
    status: 'spawning',
    terminalIndex,
  ...
  // Store teammate metadata on the agent object
  if (data._isTeammate) {
    agent._isTeammate = true;
    agent._teamName = data._teamName;
    agent._tmuxPaneId = data._tmuxPaneId;
  }
  ...
  // Set auto-expiration timer (longer for teammates and team agents)
  const lifetime = (data._isTeammate || source === 'team') ? TEAMMATE_MAX_LIFETIME : AGENT_MAX_LIFETIME;
  const timer = setTimeout(() => {
    if (activeAgents.has(id)) {
...
export function updateAgentTask(data) {
  ...
  if (agentId && activeAgents.has(agentId)) {
    ...
    agent.taskDescription = sanitizeDescription(taskDescription || prompt || description) || agent.taskDescription;
    if (subagentType) {
...
export function initWebSocket(server, options = {}) {
  ...
  sessionWss.on('connection', (ws) => {
    console.log('[session] WebSocket client connected');
    if (options.onSessionConnection) {
      options.onSessionConnection(ws);
    }
  });
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    } else if (url.pathname === '/ws/sessions') {
      sessionWss.handleUpgrade(request, socket, head, (ws) => {
        sessionWss.emit('connection', ws, request);
      });
    } else if (url.pathname === '/ws/missions' && options.onMissionUpgrade) {
      options.onMissionUpgrade(request, socket, head);
    } else if (url.pathname === '/ws/projects' && options.onProjectUpgrade) {
      options.onProjectUpgrade(request, socket, head);
    } else {
      socket.destroy();
    }
  });
  ...
  console.log('[session] WebSocket server initialized at /ws/sessions');
  console.log('[missions] WebSocket server initialized at /ws/missions');
  return wss;
}

// ============================================================
// Team Scanner Integration
// ============================================================

/**
 * Sync tmux teammates: spawn Jedi for new ones, remove Jedi for dead ones
 */
function syncTeammates() {
  try {
    const liveTeammates = getAllLiveTeammates();
    const liveIds = new Set(liveTeammates.map(t => t.agentId));

    // Spawn Jedi for newly discovered teammates
    for (const teammate of liveTeammates) {
      if (!trackedTeammates.has(teammate.agentId)) {
        console.log(`[activity] Discovered tmux teammate: ${teammate.agentId} (${teammate.type})`);
        trackedTeammates.add(teammate.agentId);
        spawnAgent({
          agentId: teammate.agentId,
          agentType: teammate.type,
          taskDescription: teammate.taskDescription,
          _isTeammate: true,
          _teamName: teammate.teamName,
          _tmuxPaneId: teammate.tmuxPaneId
        });
      }
    }

    // Remove Jedi for teammates whose panes are gone
    for (const trackedId of trackedTeammates) {
      if (!liveIds.has(trackedId)) {
        console.log(`[activity] Tmux teammate departed: ${trackedId}`);
        trackedTeammates.delete(trackedId);
        completeAgent({ agentId: trackedId, status: 'departed' });
      }
    }
  } catch (err) {
    console.error('[activity] Team scan error:', err.message);
  }
}

/**
 * Start the periodic team scanner
 */
function startTeamScanner() {
  ...
  // Periodic scan
  teamScanTimer = setInterval(syncTeammates, TEAM_SCAN_INTERVAL);
  console.log(`[activity] Team scanner started (every ${TEAM_SCAN_INTERVAL / 1000}s)`);
}

/**
 * Stop the team scanner
 */
function stopTeamScanner() {
  if (teamScanTimer) {
    clearInterval(teamScanTimer);
    teamScanTimer = null;
    console.log('[activity] Team scanner stopped');
  }
}

/**
 * Check if there are any active agents (used by server auto-shutdown)
 * @returns {boolean} True if agents are currently active
 */
export function hasActiveAgents() {
  return activeAgents.size > 0;
}

// Start team scanner after state is loaded
startTeamScanner();
 ...
 * Called by PreToolUse[Task] hook to store task metadata for correlation
 */
export async function handleTaskMetadata(req, res) {
  try {
    const { sessionId, subagentType, description } = req.body;

    if (!sessionId || !sessionId.trim()) {
      return sendJson(res, 400, { error: 'sessionId required' });
    }

    taskMetadata.set(sessionId, {
      subagentType,
      description,
      timestamp: Date.now()
    });

    console.log(`[activity] Stored task metadata for session ${sessionId}: ${subagentType}`);
    sendJson(res, 200, { success: true });
  } catch (error) {
    console.error('[activity] Metadata storage error:', error);
    sendJson(res, 500, { error: 'Failed to store metadata' });
  }
}
...
export function registerActivityRoutes(router) {
  ...
  router.post('/api/activity/metadata', handleTaskMetadata);
...
// Periodic cleanup of stale task metadata
const metadataCleanupTimer = setInterval(() => {
  const now = Date.now();
  for (const [id, data] of taskMetadata.entries()) {
    if (now - data.timestamp > METADATA_TTL) {
...
/**
 * Clean up all timers and WebSocket connections for graceful shutdown
 */
export function shutdown() {
  // Stop team scanner
  stopTeamScanner();

  // Clear metadata cleanup timer
  clearInterval(metadataCleanupTimer);

  // Clear all agent expiration timers
  expirationTimers.forEach(timer => clearTimeout(timer));
  expirationTimers.clear();
  ...
  console.log('[activity] Shutdown complete');
}
