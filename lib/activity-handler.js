...
export function spawnAgent(data) {
  const { agentId, agentType, taskDescription, prompt, subagentType, sessionId } = data;
  ...
  const agent = {
    ...
    taskDescription: (metadata?.description) || taskDescription || prompt || 'Working on task...',
    status: 'spawning',
    terminalIndex,
  ...
  // Set auto-expiration timer (longer for teammates)
  const lifetime = data._isTeammate ? TEAMMATE_MAX_LIFETIME : AGENT_MAX_LIFETIME;
