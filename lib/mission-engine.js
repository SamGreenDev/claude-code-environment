...
class MissionEngine extends EventEmitter {
  ...
  async startMission(missionId, contextOverrides = {}) {
    ...
    // Resolve workdir to absolute path, or auto-assign a default output directory
    if (!resolvedContext.workdir) {
      const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT || resolve(new URL('..', import.meta.url).pathname);
      const slug = (mission.name || missionId)
        .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 60);
      const dateStr = new Date().toISOString().slice(0, 10);
      const missionDir = join(pluginRoot, 'mission-outputs', slug);
      try { mkdirSync(missionDir, { recursive: true }); } catch { /* best effort */ }

      // Find next available version: YYYY-MM-DD, YYYY-MM-DD_v2, YYYY-MM-DD_v3, ...
      let outputDir = join(missionDir, dateStr);
      if (existsSync(outputDir)) {
        let v = 2;
        while (existsSync(join(missionDir, `${dateStr}_v${v}`))) v++;
        outputDir = join(missionDir, `${dateStr}_v${v}`);
      }
      resolvedContext.workdir = outputDir;
    }
    {
    ...
    for (const node of nodes) {
      ...
      if (taskStatus === 'completed') {
        ...
        if (taskData.activeForm && taskData.activeForm !== state._lastActiveForm) {
          await store.updateNodeState(runId, node.id, { _lastActiveForm: taskData.activeForm, _lastCommsTime: Date.now() });
          this.emit('message_logged', {
            runId,
            nodeId: node.id,
            level: 'INFO',
        ...
        if (messageCount > lastMsgCount) {
          await store.updateNodeState(runId, node.id, { _lastMsgCount: messageCount, _lastCommsTime: Date.now() });
        ...
        // Heartbeat: emit a periodic "still working" message when no activeForm/message
        // updates have surfaced for a while, so the user knows the agent is alive.
        const HEARTBEAT_INTERVAL = 60_000; // 1 minute
        const lastCommsTime = state._lastCommsTime || (state.startedAt ? new Date(state.startedAt).getTime() : 0);
        const now = Date.now();
        const sinceLastComms = now - lastCommsTime;
        if (sinceLastComms >= HEARTBEAT_INTERVAL) {
          const elapsedMin = Math.floor((now - new Date(state.startedAt).getTime()) / 60_000);
          const elapsedStr = elapsedMin < 1 ? '<1m' : `${elapsedMin}m`;
          this.emit('message_logged', {
            runId,
            nodeId: node.id,
            level: 'INFO',
            message: `Still working... (${elapsedStr} elapsed)`,
          });
          await store.updateNodeState(runId, node.id, { _lastCommsTime: now });
        }
