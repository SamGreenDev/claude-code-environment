 ...
 */
const MAX_BODY_BYTES = 1024 * 1024; // 1MB — generous for JSON config payloads
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (body.length > MAX_BODY_BYTES) {
        // Destroy the stream to stop further reads
        req.destroy();
...
/**
 * Strip path separators and ".." from user-supplied IDs to prevent
 * directory traversal when IDs are joined into filesystem paths.
 */
