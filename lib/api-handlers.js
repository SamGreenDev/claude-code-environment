...
import { sendJson, sanitizeId } from './router.js';
 ...
 * PUT /api/settings
 */
// Keys the dashboard is allowed to write into settings.json.
// Prevents accidental corruption of Claude Code config via arbitrary keys.
const SETTINGS_ALLOWLIST = new Set([
  'env', 'permissions', 'enabledPlugins', 'hooks',
  'weeklyTokenLimit', 'weeklyResetDate', 'projects',
]);

export async function handleUpdateSettings(req, res) {
  try {
    const unknownKeys = Object.keys(req.body).filter(k => !SETTINGS_ALLOWLIST.has(k));
...
export async function handleGetProjectMemory(req, res) {
  try {
    const { id } = req.params;
    const files = await getProjectMemory(sanitizeId(decodeURIComponent(id)));
