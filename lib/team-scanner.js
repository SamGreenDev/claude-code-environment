...
export function extractTeamMembers(team) {
  ...
  for (const member of team.members) {
    ...
    // Extract task description from the first line of the prompt
    let taskDescription = member.prompt || 'Working on task...';
    const firstLine = taskDescription.split('\n').find(line => line.trim()) || taskDescription;
    if (firstLine.length > PROMPT_MAX_LENGTH) {
      taskDescription = firstLine.substring(0, PROMPT_MAX_LENGTH) + '...';
    } else {
